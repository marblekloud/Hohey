    ...
    Pausable
{
    ...
    modifier onlyOwner() {
        require(
            hasRole(OWNER_ROLE, msg.sender) || msg.sender == stakingAddress
        );
        _;
    }
    ...
    function changeMinimumStake(uint256 newMinAmount) public onlyOwner {
    ...
    */
    ...
    function stake(uint256 amount) public payable {
        require(
            amount >= minStake,
            "Must have a staked balance of more than minStake MAI to become a staker"
        );
        require(
            hasRole(STAKER_ROLE, msg.sender) == false,
    ...
    */
    ...
    function withdraw(uint256 amount) public payable {
        require(
            ...
            "Must have staker role to perform this action"
        );
        require(
            amount <= stakedBalance[msg.sender],
            "Must withdraw amount within balance"
        );
        require(
            stakedBalance[msg.sender].sub(amount) >= minStake,
            "A balance of higher than minStake is required to maintain role of staker, if you wish to revoke the role of staker, use revokeStaker() function"
        );
        require(
            checkParticipation(msg.sender) == false,
    ...
    */
    ...
    function viewStakedBalance() public view returns (uint256 balance) {
        require(
            hasRole(STAKER_ROLE, msg.sender),
            "Must have staker role to perform this action"
        );
    ...
    */
    ...
    function viewStakedTotal() public view returns (uint256 balance) {
        require(
            hasRole(STAKER_ROLE, msg.sender),
            "Must have staker role to perform this action"
        );
    ...
    */
    ...
    function startPoll(
        ...
        require(
            ((_quorumOption == false && _voteQuorum == 0) ||
                (_quorumOption == true && _voteQuorum > 0)),
            "Quorum must either be allowed with a vote quorum number or disabled without a vote quorum number"
        );
        require(
            (_action == 3 && _sender != address(0)) ||
                (_action != 3 && _sender == address(0)),
            "Transferring tokens require address of sender, not transferring tokens must leave address of sender as 0"
        );
        require(
            _votedTokenAmount > 0,
    ...
    */
    ...
    function claimVote(uint256 pollID, address voter)
        public
        checkTime(pollID)
        voterCheck(voter)
    {
        // msg.sender is usually for contracts, not addresses. So here, it is change(d) to voter
        require(voter != address(0));
        uint256 claimable = (
            stakedTotal[voter].sub(pollMapping[pollID]._votesClaimed[voter])
        )
        .sub(minStake.sub(1));
        require(claimable > 0, "Address has no votes available to claim");
        pollMapping[pollID]._votesClaimed[voter] = pollMapping[pollID]
        ._votesClaimed[voter]
        .add(claimable);
        pollMapping[pollID]._votes[voter] = pollMapping[pollID]
        ._votes[voter]
        .add(claimable);
    ...
    */
    ...
    function numOfVotes(address voter, uint256 pollID)
        public
        checkTime(pollID)
    ...
    */
    ...
    function makeVote(
        ...
        uint256 side,
        address voter
    ) public checkTime(pollID) voterCheck(voter) {
        require(voter != address(0), "Must be a non-zero address");
        require(pollMapping[pollID].ongoing == true, "Must be an ongoing poll");
        require(
            pollMapping[pollID]._votes[voter] > 0,
            "Address has no votes currently, call function claimVote to claim a vote"
        );
        require(
            votesMade <= pollMapping[pollID]._votes[voter],
            "You do not have enough votes to do this action"
        );
        require(
            pollMapping[pollID]._side[voter] == 0 ||
                pollMapping[pollID]._side[voter] == side,
            "You have voted for another side before"
        );
        require(side == 1 || side == 2, "Invalid side");
        if (side == 1) {
            pollMapping[pollID].votesFor = pollMapping[pollID].votesFor.add(1);
        } else if (side == 2) {
            pollMapping[pollID].votesAgainst = pollMapping[pollID]
            .votesAgainst
            .add(1);
        }
        pollMapping[pollID]._side[voter] = side;
    ...
    */
    ...
    function calculateResults(uint256 pollID) public checkTime(pollID) {
        ...
        if (pollMapping[pollID].quorumOption == false) {
            if (
                pollMapping[pollID].votesFor > pollMapping[pollID].votesAgainst
            ) {
    ...
    */
    ...
    function quorumCalc(uint256 pollID)
        public
        checkTime(pollID)
    ...
    */
    ...
    function endPoll(uint256 pollID) public onlyOwner checkTime(pollID) {
    ...
    */
    ...
    function _claimRewards(uint256 pollID) public checkTime(pollID) {
        calculateResults(pollID);
        require(pollMapping[pollID].passed == true, "the poll did not pass");
        require(
            pollMapping[pollID].rewardsClaimed == false,
    ...
    */
    ...
    function mintRewards(uint256 pollID)
        public
        payable
        checkTime(pollID)
        onlyOwner
    {
        ...
        require(
            pollMapping[pollID].action == 1,
            "The poll is not made for minting"
        );
        require(
            pollMapping[pollID].ongoing == false,
            "The poll is still ongoing"
        ...
        _beforeTokenTransfer(
            address(0),
            msg.sender,
            pollMapping[pollID].votedTokenAmount
        );
        _mint(
            pollMapping[pollID].toAddress,
            pollMapping[pollID].votedTokenAmount
        );
    ...
    */
    ...
    function burnRewards(uint256 pollID)
        public
        payable
        checkTime(pollID)
        onlyOwner
    ...
    function transferRewards(uint256 pollID)
        public
        virtual
        checkTime(pollID)
        onlyOwner
    {
        ...
        require(pollMapping[pollID].ongoing == false);
        _transfer(
            pollMapping[pollID].fromAddress,
            pollMapping[pollID].toAddress,
            pollMapping[pollID].votedTokenAmount
        );
    ...
    function takeSnapshot(uint256 pollID) public virtual onlyOwner {
    ...
    */
    ...
    function checkParticipation(address voter)
        public
        returns (bool participating)
    {
        for (uint256 i = 1; i <= pollNonce; i++) {
            correctTime(i);
            if (
                pollMapping[i].ongoing == true &&
                pollMapping[i]._participant[voter] == true
            ) {
                return true;
            }
        }
